...
| Known limitation: drums SP activation auto-hits do not raise the live percent | On drums, a star power activation note the player skips is auto-hit inside the engine with no `OnNoteHit` dispatch, so the live percent for the section containing it can never reach 100% while that section is current. The block's state and the end-of-song scan are unaffected: the block still goes clean on entry and is only dropped by a real miss, and the scan reads `WasHit` off the notes rather than counting dispatches. Fixing it would need a change in `YARG.Core`, which this fork does not modify. |
