...
Principle: **state-bearing readouts are exact, one-shot transients are skipped.** The re-simulation runs under the existing seek-suppression flag (`GameManager.IsSeekingReplay` or a rewind equivalent) so hit and miss SFX, haptics, camera punches and Star Power award/ready SFX do not re-fire while the recorded inputs replay. A fade of about 0.3 s with the rewind SFX covers the swap: everything is reset while hidden, and fades back in at lead-in start. Per-element rules are tabulated below.

## Score page badge

A `REWOUND` pill in the score card's Engine Settings strip, beside `DEFAULT ENGINE` and `MODIFIERS ACTIVE`, styled as a `ColoredPillElement` with an amber tint (mockup uses the `#FFB020` family, derived from the game's orange `#FF8413`). No modifier icon, no bottom-tag change, no rewind count. It is **session-only**: nothing is persisted on the score record, so a history entry or a saved replay of a rewound run is indistinguishable from a normal run.
...
| Section strip | Sections before the target keep their status; target and everything after go back to the unplayed look. The highlight sits on the target from the moment the rewind lands, through the lead-in. No rewind marker on the strip. Needs the new `SectionStripState` rewind. |
...
Only what the tickets deliberately left to implementation: whether the section strip rewinds by rebuild (`Create` + `SetSectionState`) or by a `RewindTo(index)` cursor regression; the exact fade length (about 0.3 s) and curve; how much of the background-video seek hitch the fade actually masks, and whether it must hold until the video handshake completes; the amber text/tint pair for the `REWOUND` pill, which has no existing tag colour set, and whether the 431-wide Engine Settings strip needs a smaller font or tighter spacing to fit a third pill (the existing two already wrap when one is added).
