...
| Engine | Frozen for the whole lead-in via the existing `GameManager.Rewinding` gate, holding the restored section-start state. Judged notes before the marker do not respawn (see the HUD table); only a sustain crossing the boundary is shown, in held state. Inputs arriving during the window are **dropped**, not queued for resume the way `SendInputsOnResume` queues them on the unpause path - **except inside a grace window just before the marker**. Corrected in implementation: the drop is on the input's timestamp, not on the lead-in flag. A note sitting close after the marker is legitimately hit *before* it (the front end of the hit window, plus the strum leniency on guitar), so a flat drop made the section's first note unhittable whenever it landed within that distance of the countdown - and the frame the marker is crossed on was lost outright, because `InputManager.UpdateBindingsForFrame` runs in EarlyUpdate, ahead of `UpdateRewindLeadIn`. An input stamped at or after `marker - BasePlayer.LeadInInputGrace` is queued to the engine normally; the engine is frozen at the marker and `BaseEngine.QueueInput` clamps it forward, so it is judged as a press at the marker. The grace is per player, derived from the engine parameters: `|HitWindow.GetFrontEnd(MaxWindow)|` plus guitar's `StrumLeniency` (scaled by `HitWindow.Scale`, the song speed, since the engine scales the leniency timer the same way while the front end is already scaled), plus 0.02 s of fixed slack for the frame boundary, capped at 0.25 s (which is also what `InfiniteFrontEnd` takes). **A Star Power press during the countdown is ignored**, grace window or not: the grace exists so the section's first note is hittable, not so a tap on the countdown can deploy the instant the marker lands. A real pause taken inside the window is excluded too, since the clock stops but input-system time does not; and because the frozen engine's input queue cannot be emptied from the game assembly, a restart that follows a grace input discards it by rebuilding and re-simulating the engines exactly as the rewind does. The engine goes live at the marker. |
...
3. **Lead-in freeze with inputs dropped.** Hold the engine under `GameManager.Rewinding` from the landing until song time reaches the marker, discarding inputs received in the window rather than queueing them - except those inside the per-player grace window before the marker (`GameManager.ShouldDropLeadInInput` / `BasePlayer.LeadInInputGrace`), which are queued to the frozen engine and marked in `RewoundEngineInputState` so the marker resend does not duplicate them.
